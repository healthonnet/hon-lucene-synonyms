 ...
 * @lucene.experimental
 */
public class SynonymExpandingExtendedDismaxQParserPlugin extends org.apache.solr.search.QParserPlugin {
    public static final String NAME = "synedismax";
...
class ExtendedDismaxQParser extends QParser {
    ...
    private Query attemptToAddSeparateSynonymQuery(ExtendedSolrQueryParser up, String mainUserQuery,
        ...
        for (int i = 0;;i++) {
            ...
            up.setShouldKeepSynonym(i);
            Query synonymQuery = up.parse(mainUserQuery);
            
            // empty query as last one, i.e. no synonyms, so ignore
            if (synonymQuery.toString().equals("")) {
                break;
            }
    ...
    private void addShingledPhraseQueries(final BooleanQuery mainQuery, final List<Clause> clauses,
         ...
         * Yonik first wrote the ExtendedDismaxQParserPlugin, he added the
    ...
    class ExtendedSolrQueryParser extends SolrQueryParser {
        ...
        int minClauseSize = 0; // minimum number of clauses per phrase query...
                               ...
                               // via sloppy phrases
        boolean exceptions; // allow exceptions to be thrown (for example on a
                            // missing field)
        ...
        public void setShouldKeepSynonym(int indexToKeep) {
            analyzer.shouldKeepSynonym = indexToKeep;
        }
...
final class ExtendedAnalyzer extends Analyzer {
    ...
    public int shouldKeepSynonym = -1;
    ...
    private Analyzer removeSynonymFilter(String fieldName, Analyzer qa) {
        ...
        for (int i = 0; i < filterFactories.length; i++) {
            ...
            if (tf instanceof TuneableSynonymFilterFactory) {
                ((TuneableSynonymFilterFactory)tf).setIndexToKeep(shouldKeepSynonym);
                if (!removeSynonymFilter) {
                    newTokenFilterFactories.add(tf);
                }
            } else if (tf instanceof ShingleFilterFactory) {
                if (!removeSynonymFilter) {
                    newTokenFilterFactories.add(tf);
                }
            } else {
